...
export default function PostEditorForm({
  ...
  const requestExcerptSuggestion = async ({
    silent = false,
  }: {
    silent?: boolean;
  } = {}) => {
    if (excerptLoading) return;
    const titleValue = title.trim();
    const contentValue = markdown.trim();
    if (!titleValue && !contentValue) {
      if (!silent) flashSystemMsg("Provide title or content first.");
      return;
    }

    setExcerptLoading(true);
    try {
      const response = await fetch("/api/excerpt/suggest", {
        method: "POST",
      ...
      const data = (await response.json()) as { excerpt?: string | null; error?: string };
      if (!response.ok) {
      ...
      if (!suggestion) {
        if (!silent) {
          flashSystemMsg("No excerpt available yet.");
        }
        return;
      }

      setExcerpt(suggestion);
      if (!silent) {
        flashSystemMsg("Excerpt generated.");
      }
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Failed to generate excerpt.";
      if (!silent) {
  ...
  useEffect(() => {
    if (!autoGenerateExcerpt || autoGeneratedRef.current) return;
    autoGeneratedRef.current = true;
    void requestExcerptSuggestion();
  }, [autoGenerateExcerpt]);
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <label
              id="excerpt"
              className="text-xs uppercase tracking-[0.3em] app-muted"
            >
              Excerpt
            </label>
            <button
              type="button"
              onClick={() => void requestExcerptSuggestion()}
              disabled={excerptLoading}
              className="inline-flex items-center justify-center border app-border px-2 py-1 text-[10px] uppercase tracking-[0.25em] app-muted hover:text-[color:var(--accent)] disabled:opacity-50 transition"
            >
              {excerptLoading ? "Generating..." : "AI Summarize"}
            </button>
          </div>
          <input
            name="excerpt"
